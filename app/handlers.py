...
async def catalog(message: Message):
    await message.answer('Выберите категорию курсов:',
...
async def handle_user_message(message: Message):
    if message.chat.type == "private" and message.text not in ['Контакты' + emoji.emojize(":globe_with_meridians:"),
                                                               'Связаться с нами' + emoji.emojize(
                                                                   ':telephone_receiver:')]:
        user_text = f"{hbold('Сообщение от пользователя')} {message.from_user.username} ({message.from_user.id}):\n\n{hitalic(message.text)}"

        try:
            await message.bot.send_message(os.getenv("ADMIN_ID"), user_text, parse_mode='HTML')
...
async def handle_name(callback: CallbackQuery):
    course_slug = callback.data.split("_")[1]
    course = await rq.get_course_by_slug(course_slug)
    if course:
        course_info = (
            f"Название: {course['title']}\n"
            f"Подробнее: {course['url']}"
        )
        await callback.message.answer(course_info)
    else:
